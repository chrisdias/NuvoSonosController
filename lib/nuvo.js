...
function Nuvo(discovery) {
  ...
  sp.on('data', function (data) {
    ...
    if (data.search("HNEXT") > 0 || data.search("HPREV") > 0) {
      // we want to show the favorites menu
      menuState.playerState = 'favMenu';
    }
    
    // HPREV - Hard Previous, go into Fav menu mode
    // HNEXT - Hard Next, go into Fav menu mode
    // NEXT - Next
    // PREV - Previous
    // HPLAYPAUSE - Stop
    ...
    if (data.search("PLAYPAUSE") > 0) {
      if (menuState.playerState === 'favMenu') {
        console.log("in fav menu");
        console.log("this means i would want to select the favorite");
        console.log("then i would set playerState");
        menuState.playerState = 'normal';
      } else {
        console.log("in normal mode");
      }
      
      if (players[0].getState().playerState === 'PLAYING') {
        /// pause
        players[0].pause(); 
      } else {
        /// play
    ...
    if (data.search("S2NEXT") > 0) {
      ...
      if (menuState.playerState === 'favMenu') {
        console.log("in fav menu");
        console.log("this means i would want to show the NEXT favorite...");
      } else {
        console.log("in normal mode");
      }
      ...
      if (menuState.playerState === 'favMenu') {
        console.log("in fav menu");
        console.log("this means i would want to show the PREVIOUS favorite...");
      } else {
        console.log("in normal mode");
      }
